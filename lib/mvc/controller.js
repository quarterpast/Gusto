(function(){
  var util, pathutil, Walk, signal, async, Log, Sync, ControllerSupport, __slice = [].slice;
  util = require('util');
  pathutil = require('path');
  Walk = require("./loader").Walk;
  ...
  exports.ControllerLoader = function(dir){
    ...
    return out;
    function __fn(exp){
      var action, id, __ref, __len, __results = [];
      for (action = 0, __len = (__ref = Paths(exp)).length; action < __len; ++action) {
        id = __ref[action];
        __results.push(out[id] = action);
      }
      return __results;
    }
  };
