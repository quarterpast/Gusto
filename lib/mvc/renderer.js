(function(){
  var Config, async, path, exists, Renderer, __ref;
  __ref = require("../main"), Config = __ref.Config, async = __ref.async;
  path = require('path');
  ...
  exports.Renderer = Renderer = (function(){
    ...
    Renderer.resolve = async(function(file){
      var ext, eng, f, filename, __ref, __i, __ref1, __len;
      if (ext = path.extname(file)) {
        return this.subclasses[ext] || (function(){
          throw Error("No template for " + file);
        }());
      }
      for (ext in __ref = this.subclasses) {
        eng = __ref[ext];
        for (__i = 0, __len = (__ref1 = [file + "/index." + ext, file + "." + ext]).length; __i < __len; ++__i) {
          f = __ref1[__i];
          filename = path.join(Config.templatePath, f);
    ...
    prototype.compile = function(file){
      this.file = file;
    };
    prototype.partial = function(part){
      return Renderer.resolve(path.resolve(this.file, part));
    };
    function Renderer(){}
    return Renderer;
  }());
