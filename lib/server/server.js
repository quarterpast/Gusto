(function(){
  ...
  exports.Server = Server = (function(){
    ...
    Server.error = false;
    Server.hijack = function(promise, err){
      var __this = this;
      this.error = err;
      return promise.then(function(){
        return __this.error = false;
      });
    };
    prototype.serve = function(request){
      ...
      out = Q.defer();
      Sync(function(){
        var time, res, get, post, action, params, __ref;
        ...
        try {
          ...
          request.get = get;
          request.post = post;
          __ref = Router.route(request).filter(function(it){
            return !(it instanceof NotFound);
          })[0], action = __ref.action, params = __ref.params;
          ...
          out.resolve(__import({
            ...
            status: 200,
            onclose: __bind(time, 'end')
          }, constructor.error && (!("content-type" in (res.headers || (res.headers = {}))) || res.headers["content-type"] === "text/html")
            ? constructor.error
            : 'forEach' in res ? {
              body: res
            } : res));
