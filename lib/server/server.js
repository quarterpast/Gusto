(function(){
  var Config, Log, async, Q, http, url, querystring, vm, fs, util, Sync, Router, Errors, Loader, SyncPromise, Timer, Server, __ref;
  ...
  util = require('util');
  Sync = require('sync');
  __ref = require("./router"), Router = __ref.Router, Errors = __ref.Errors;
  ...
  exports.Server = Server = (function(){
    ...
    function Server(port, host){
      var __this = this;
      Sync(function(){
        __this.router = new Router;
        __this.server = http.Server(__bind(__this, 'serve'));
        Log.log("listening on %s:" + port, host || "*");
        return __this.server.listen(port, host);
      }, __bind(this, 'error'));
    }
    return Server;
  }());
