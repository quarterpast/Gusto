(function(){
  ...
  exports.Server = Server = (function(){
    ...
    Server.oldErrs = [];
    Server.displayErr = 0;
    Server.hijack = function(id, err){
      var __ref, __this = this;
      this.errors[id] = err;
      this.oldErrs.push(this.displayErr);
      this.displayErr = id;
      return (__ref = err.promise) != null ? __ref.then(function(it){
        var i, id, __ref, __len, __results = [];
        delete __this.errors[it];
        for (i = 0, __len = (__ref = __this.oldErrs).length; i < __len; ++i) {
          id = __ref[i];
          if (id === it) {
            __results.push(__this.oldErrs.splice(i, 1));
          }
        }
        return __results;
      }) : void 8;
    ...
    prototype.serve = function(request){
      var time, res, get, post, found, route, action, params, __i, __ref, __len;
      ...
      try {
        ...
        return __import({
          ...
          status: 200,
          onclose: __bind(time, 'end')
        }, constructor.displayErr && (!("content-type" in (res.headers || (res.headers = {}))) || res.headers["content-type"] === "text/html")
          ? constructor.errors[constructor.displayErr]
          : 'forEach' in res ? {
            body: res
          } : res);
      }
