(function(){
  var Config, signal, url, flip, each, methods, everyMethod, NotFound, Aliases, Route, Router, __ref;
  Config = require("../main").Config;
  signal = require("../mvc/signal").signal;
  url = require('url');
  __ref = require('prelude-ls'), flip = __ref.flip, each = __ref.each;
  methods = ['HEAD', 'GET', 'POST', 'PUT', 'TRACE', 'DELETE', 'OPTIONS', 'PATCH'];
  everyMethod = flip(each)(methods);
  ...
  exports.Route = Route = (function(){
    ...
    function Route(action, path, method){
      this.action = action;
      this.path = path;
  ...
  exports.Router = Router = (function(){
    ...
    var register, prototype = Router.prototype, constructor = Router;
    Router.routers = [];
    ...
    register = __curry(function(method, path, action){
      switch (false) {
      case !__in(method.toUpperCase(), [('*')].concat(methods)):
        throw Error('unimplemented');
      default:
    ...
    prototype.add = function(path, action){
      var m, __i, __ref, __len, __j, __ref1, __len1;
      if (action.aliases != null) {
        for (__i = 0, __len = (__ref = action.aliases[m]).length; __i < __len; ++__i) {
          path = __ref[__i];
          for (__j = 0, __len1 = (__ref1 = methods).length; __j < __len1; ++__j) {
            m = __ref1[__j];
            register(m, path, action);
          }
        }
      }
      return register('*', path, action);
    };
    everyMethod(function(method){
      return prototype[method.toLowerCase()] = register(method);
    });
