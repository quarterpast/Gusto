(function(){
  var Config, Controllers, url, vm, methods, Script, InvalidMethod, NotFound, NoAction, Generic, Router, __slice = [].slice;
  ...
  exports.Errors.NotFound = NotFound = (function(superclass){
  ...
  exports.Router = Router = (function(){
    ...
    prototype.route = function(req){
      var uri, route, errors, keys, pieces, reg, params, id, action, _i, _ref, _len;
      uri = url.parse(req.path);
      for (_i = 0, _len = (_ref = this.compiled).length; _i < _len; ++_i) {
        route = _ref[_i];
        errors = [];
        if (!(route.method === "*" || route.method === req.method)) {
          errors.push(new InvalidMethod(route.method));
        }
        keys = [];
        ...
        if (reg.test(uri.pathname)) {
          ...
          if (!(id = route.id.runInNewContext(params))) {
            errors.push(new NoAction(id));
          } else if (!(action = Controllers.byId(id))) {
            errors.push(new NoAction(id));
          }
          return {
            action: action,
          ...
          errors.push(new NotFound(uri.pathname));
        }
      }
