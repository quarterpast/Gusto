(function(){
  ...
  fill = __curry(function(args, func){
    return __curry(function(){
      ...
      return func.apply(null, (function(){
        var __to, __results = [];
        for (i = 0, __to = fold1(max, [(params.length)].concat(keys(args))); i <= __to; ++i) {
          if ((that = args[i]) != null) {
            __results.push(that);
  ...
  exports.Router = Router = (function(){
    ...
    prototype.add = function(path, action, alias){
      alias == null && (alias = true);
      if (alias && action.aliases != null) {
        each(flip(this.add)(false, action))(
        everyMethod(objToFunc(action.aliases)));
      }
