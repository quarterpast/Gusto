(function(){
  var Config, signal, url, methods, fill, everyMethod, NotFound, Aliases, Route, Router, __slice = [].slice;
  ...
  exports.Router = Router = (function(){
    ...
    register = __curry(function(routes, method, path, action){
      var route;
      switch (false) {
      case !__in(method.toLowerCase(), [('*')].concat(methods)):
        return route = new Route(method, path, action);
      default:
        throw new Error("invalid method " + method);
      }
    });
    prototype.add = function(path, action){
      var method, paths, __i, __ref, __len, __ref1;
      if (action.aliases != null) {
        for (__i = 0, __len = (__ref = zip(methods, everyMethod(action.aliases))).length; __i < __len; ++__i) {
          __ref1 = __ref[__i], method = __ref1[0], paths = __ref1[1];
          each(fill({
            1: action
          }, register(this.routes, method)), paths);
        }
      }
      return register(this.routes, '*', path, action);
    };
    everyMethod(function(method){
      return prototype[method] = register(method);
    });
